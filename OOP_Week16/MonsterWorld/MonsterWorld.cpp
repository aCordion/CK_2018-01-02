...
MonsterWorld::MonsterWorld(int w, int h, int n) : world(h), canvas(w, h), xMax(w), yMax(h), nItemCnt(n) {
	//nMon = 0;
	nMove = 0;
	for (int y = 0; y < yMax; y++) {
	...
	string monName = "";
		...
		for (int i = 0; i < maxwalk; i++) {
			...
			if (_kbhit()) {
				...
				if (ch == 224) {
					ch = _getch();
					((Tuman*)(pMon[pMon.size() - 1]))->moveHuman(world, xMax, yMax, ch);
				}
				/*else {
