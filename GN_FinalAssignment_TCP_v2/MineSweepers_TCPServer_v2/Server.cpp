...
//가변
#define MAX_CLIENTNUM 4		//접속할 클라이언트 수 1~4 사이
//
...
int main(int argc, char ** argv) {
	...
	for (int y = 0; y < MAPSIZEY; y++) {
		for (int x = 0; x < MAPSIZEX; x++) {
			sMapData.map[y][x] = htonl(game.getMapNumdata(y, x));
			sMapData.cliNum = htonl(MAX_CLIENTNUM);
...
unsigned WINAPI Client1_Thread(void *data) {
	...
	while (true) {
		//WaitForSingleObject(h_Mutex, INFINITE);
			...
			spData.x1 = htonl(x); spData.y1 = htonl(y);
			//MutexWait
			WaitForSingleObject(h_Mutex, INFINITE);
			sData = rData[0];
			for (int i = 0; i < MAX_CLIENTNUM; i++) {
				send(hClntSock[i], (char*)&spData, sizeof(spData), 0);
				send(hClntSock[i], (char*)&sData, sizeof(sData), 0);
			}
...
unsigned WINAPI Client2_Thread(void *data) {
	...
	while (true) {
		...
		spData.x2 = htonl(x); spData.y2 = htonl(y);
		//MutexWait
		WaitForSingleObject(h_Mutex, INFINITE);
		sData = rData[1];
		for (int i = 0; i < MAX_CLIENTNUM; i++) {
...
unsigned WINAPI Client3_Thread(void *data) {
	...
	while (true) {
		...
		spData.x3 = htonl(x); spData.y3 = htonl(y);
		//MutexWait
		WaitForSingleObject(h_Mutex, INFINITE);
		sData = rData[2];
		for (int i = 0; i < MAX_CLIENTNUM; i++) {
...
unsigned WINAPI Client4_Thread(void *data) {
	...
	while (true) {
		...
		spData.x4 = htonl(x); spData.y4 = htonl(y);
		//MutexWait
		WaitForSingleObject(h_Mutex, INFINITE);
		sData = rData[3];
		for (int i = 0; i < MAX_CLIENTNUM; i++) {
