...
void Vampire::move(Vector<Vector<int>>& map, int maxx, int maxy) {
	if (!isSleep()) {
		int dir = rand() % 4;
		if (dir == 0) p[0]--;
		else if (dir == 1) p[0]++;
...
void KGhost::move(Vector<Vector<int>>& map, int maxx, int maxy) {
	if (!isSleep()) {
...
void Jiangshi::move(Vector<Vector<int>>& map, int maxx, int maxy) {
	if (!isSleep()) {
		int dir = rand() % 2;
...
void Smombi::move(Vector<Vector<int>>& map, int maxx, int maxy)
{
	if (!isSleep()) {
		int dir = rand() % 4;
...
void SuperSmombi::move(Vector<Vector<int>>& map, int maxx, int maxy)
{
	if (!isSleep()) {
		int dir = rand() % 4;
		int jump = (rand() % 5) + 1;
